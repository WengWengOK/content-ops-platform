...
/** 后端 TaskStatus → 作品中心状态 */
const STATUS_FROM_TASK: Record<string, WorkStatus> = {
  COMPLETED: 'published',
  AWAITING_HUMAN: 'draft',
  FAILED: 'draft',
  PENDING: 'generating',
...
const GRADIENTS = [
  'linear-gradient(135deg, #FF2D5E, #FF5C8A)',
  'linear-gradient(135deg, #165DFF, #4080FF)',
  'linear-gradient(135deg, #F77234, #FFAD66)',
  'linear-gradient(135deg, #7B61FF, #A78BFA)',
  'linear-gradient(135deg, #14C9C9, #5CE0E0)',
  'linear-gradient(135deg, #9FDB1D, #BEF264)',
  'linear-gradient(135deg, #E8164A, #FF5C8A)',
  'linear-gradient(135deg, #F7BA1E, #FFD666)',
]
...
function toWork(ctx: TaskContext): Work {
  const artifacts = (ctx.accumulatedArtifacts ?? {}) as Record<string, unknown>
  const topicPlan = artifacts['topic-planning'] as Record<string, unknown> | undefined
  const title =
    (ctx.inputs?.additionalContext as string) ||
    (ctx.inputs?.topic as string) ||
    ctx.accountProfile?.accountName ||
    '未命名工作流'
  const stageLabel = STAGE_LABELS[ctx.currentStage] || ctx.currentStage
  return {
    id: ctx.workflowId,
    title,
    topic: (ctx.inputs?.topic as string) || ctx.accountProfile?.niche || '未分类',
...
export function WorkCenterPage() {
  ...
  useEffect(() => {
    ...
    listWorkflows()
      .then((list) => {
        if (!cancelled) setWorks(list.map(toWork))
      })
      .catch(() => {
  ...
  // Stats computed from the real works data
  const stats = [
    {
      label: '全部作品',
      value: works.length,
    ...
    {
      label: '已发布',
      value: works.filter((w) => w.status === 'published').length,
      color: '#00B42A',
      pulse: false,
      sub: '已上线作品',
    },
    {
      label: '草稿',
      value: works.filter((w) => w.status === 'draft').length,
      color: '#FF7D00',
      pulse: false,
      sub: '待发布作品',
    },
    {
      label: '生成中',
      value: works.filter((w) => w.status === 'generating').length,
      color: '#165DFF',
      pulse: true,
      sub: '正在生成作品',
    },
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-4">
          {filtered.map((work) => {
            ...
            return (
              <Link key={work.id} to={`/workflow-detail?workflowId=${work.id}`} className="work-card">
                {/* Cover area */}
                {work.hasCover ? (
                  <div
