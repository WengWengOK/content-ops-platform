...
public interface ContentCreationAgent {
     ...
     */
    @UserMessage("""
            输出 JSON 结构必须严格为：{"title":"","outline":{"introduction":"","sections":[{"heading":"","keyPoints":"","example":""}],"conclusion":""},"writingNotes":[],"references":[],"estimatedWordCount":0,"angle":""}
            重要输出约束：你必须只返回一个合法 JSON 对象，禁止任何前言、解释或对话，禁止使用 markdown 代码块（不要 ```json 包裹）；字符串值内部严禁出现未转义的英文双引号，需要引用时一律使用中文引号「」或“”。
            【阶段一：大纲生成】
            ...
            6. 返回结构化的大纲结果
            """)
    String generateOutline(@MemoryId String memoryId,
                           @V("topic") String topic,
                           @V("angle") String angle,
                           @V("accountNiche") String accountNiche,
                           @V("targetAudience") String targetAudience,
                           @V("tone") String tone,
                           @V("additionalContext") String additionalContext,
                           @V("platform") String platform,
                           @V("platformGuidance") String platformGuidance,
                           @V("personalExperience") String personalExperience);
     ...
     */
    @UserMessage("""
            输出 JSON 结构必须严格为：{"outline":{"introduction":"","sections":[{"heading":"","keyPoints":"","example":""}],"conclusion":""},"draftContent":"","wordCount":0,"titleVariations":[],"tags":[],"summary":""}
            重要输出约束：你必须只返回一个合法 JSON 对象，禁止任何前言、解释或对话，禁止使用 markdown 代码块（不要 ```json 包裹）；字符串值内部严禁出现未转义的英文双引号，需要引用时一律使用中文引号「」或“”。
            【阶段二：初稿生成】
    ...
    ContentDraftResult generateDraft(@MemoryId String memoryId,
                                     @V("confirmedOutline") String confirmedOutline,
                                     @V("topic") String topic,
                                     @V("accountNiche") String accountNiche,
                                     @V("tone") String tone,
    ...
    String createDraft(@MemoryId String memoryId,
                       @V("topic") String topic,
                       @V("angle") String angle,
                       @V("accountNiche") String accountNiche,
                       @V("targetAudience") String targetAudience,
