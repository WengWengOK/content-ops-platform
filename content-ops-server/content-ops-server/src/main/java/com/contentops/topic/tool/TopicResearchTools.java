...
import com.contentops.trend.TrendHotspot;
...
public class TopicResearchTools {
    ...
    /**
     * 从独立热点监控模块取多平台最新热榜，选题 Agent 直接基于热点生成选题。
     */
    @Tool("获取指定平台的最新热点列表（选题用，返回平台/排名/标题/热度/分类）")
    public String getTrendingHotspots(String platform, Integer limit) {
        log.info("[Tool] getTrendingHotspots invoked: platform={}, limit={}", platform, limit);
        int safeLimit = limit == null || limit <= 0 ? 10 : Math.min(limit, 20);
        java.util.List<TrendHotspot> hotspots = trendService.listLatest(platform, safeLimit);
        if (hotspots.isEmpty()) {
            return "暂无可用的热点数据，请稍后重试或换一个平台";
        }
        StringBuilder sb = new StringBuilder("最新热点列表（共 ").append(hotspots.size()).append(" 条）：\n");
        for (TrendHotspot h : hotspots) {
            sb.append("- [").append(h.getPlatform());
            if (h.getRank() != null) {
                sb.append(" #").append(h.getRank());
            }
            sb.append("] ").append(h.getTitle());
            if (h.getHeat() != null) {
                sb.append("（热度 ").append(h.getHeat()).append("）");
            }
            if (h.getCategory() != null && !h.getCategory().isBlank()) {
                sb.append(" · ").append(h.getCategory());
            }
            if (h.getAnalysis() != null) {
                if (h.getAnalysis().getRelevance() != null) {
                    sb.append(" · 相关度 ").append(h.getAnalysis().getRelevance());
                }
                if (h.getAnalysis().getCredibility() != null) {
                    sb.append(" · 可信度 ").append(h.getAnalysis().getCredibility());
                }
                if (h.getAnalysis().getSummary() != null && !h.getAnalysis().getSummary().isBlank()) {
                    sb.append(" · 摘要：").append(h.getAnalysis().getSummary());
                }
                if (Boolean.TRUE.equals(h.getAnalysis().getRiskFlag())) {
                    sb.append(" ⚠️ 疑似夸大/谣言，慎用");
                }
            }
            sb.append("\n");
        }
        sb.append("请基于这些热点生成选题方向，优先选择与账号定位匹配的话题。");
        return sb.toString();
    }
}
