 ...
 *
 * <p>Actual bean is built programmatically in {@link com.contentops.publish.config.PublishAgentConfig} using
...
public interface PublishingAgent {
     ...
     */
    @UserMessage("""
            不要调用任何工具，直接基于文章内容完成多平台排版并返回结构化 JSON。
            输出 JSON 结构必须严格为：{"publications":[{"platform":"","articleUrl":"","formattedContent":"","status":"PUBLISHED","failureReason":"","publishedAt":"2026-01-01T00:00:00","platformMetadata":{}}],"status":"SUCCESS"}
    ...
    PublishResult formatAndPublish(@MemoryId String memoryId,
                                   ...
                                   @V("coverImageUrl") String coverImageUrl,
                                   @V("tone") String tone);
}
