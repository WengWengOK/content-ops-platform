...
public class KnowledgeBaseService {
    ...
    /** 存储模式：pgvector / in-memory / unavailable */
    private String storeMode = "unavailable";
    ...
    public void init() {
        ...
        try {
            ...
            log.warn("PGVector embedding store unavailable, will try in-memory fallback: {}",
                    e.getMessage());
            this.embeddingStore = null;
        }
        // 真实接入：PGVector 不可用时（如本机 H2 开发环境）回退到进程内内存向量库，
        // 本地 BGE 嵌入 + 余弦相似度照常工作，向量检索不再是「纸面能力」。
        if (!vectorReady && properties.isInMemoryFallback()) {
            try {
                if (this.embeddingModel == null) {
                    this.embeddingModel = new BgeSmallZhV15QuantizedEmbeddingModel();
                }
                this.embeddingStore = new dev.langchain4j.store.embedding.inmemory.InMemoryEmbeddingStore<>();
                this.storeMode = "in-memory";
                log.info("In-memory embedding store fallback enabled (dimension={})",
                        embeddingModel.dimension());
            } catch (Exception e2) {
                log.error("Failed to initialize in-memory embedding store fallback", e2);
                this.embeddingStore = null;
    ...
    /** 当前向量存储模式：pgvector / in-memory / unavailable */
