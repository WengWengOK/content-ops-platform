...
public final class RequestInputResolver {
    ...
    /**
     * 从指定阶段（如 "topic-planning"）的产物 Map 中读取嵌套字段。
     *
     * <p>真实 Agent 调用时，阶段产物以 Map 形式挂在
     * {@code accumulatedArtifacts[stageCode]} 下（例如
     * {@code accumulatedArtifacts["topic-planning"]["angle"]}），
     * 顶层 {@link #resolve} 读不到，需按阶段定位。
     */
    public static String resolveFromStage(AgentTaskRequest request, String stageCode, String key) {
        if (request == null || stageCode == null || key == null
                || request.getAccumulatedArtifacts() == null) {
            return null;
        }
        Object stageData = request.getAccumulatedArtifacts().get(stageCode);
        if (stageData instanceof Map<?, ?> map && map.containsKey(key)) {
    ...
    /**
     * 解析参数；为空时返回空串，避免 LangChain4j 模板对 null 变量报错。
     */
    public static String resolveOrEmpty(AgentTaskRequest request, String key) {
        String value = resolve(request, key);
