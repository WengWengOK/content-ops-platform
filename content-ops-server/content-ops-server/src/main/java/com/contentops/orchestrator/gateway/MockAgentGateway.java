...
@ConditionalOnProperty(name = "CONTENTOPS_MODE", havingValue = "mock")
public class MockAgentGateway implements AgentGateway {
    ...
    public AgentResponse<Map<String, Object>> callContentDraft(AgentTaskRequest request) {
        ...
        data.put("content", String.format(
                ...
                "从今天开始试试吧！有问题评论区见～",
                niche, audience));
        // 聊天续改：若请求带 modificationRequest，则把修改意见追加进草稿，便于 mock 模式看到变化
        Object modificationRequest = request.getInputs() != null
                ? request.getInputs().get("modificationRequest")
                : null;
        if (modificationRequest != null && !String.valueOf(modificationRequest).isBlank()) {
            data.put("content", String.valueOf(data.get("content"))
                    + "\n\n---\n\n> ✏️ 已根据你的修改意见更新：" + modificationRequest + "\n");
        }
    ...
    public AgentResponse<Map<String, Object>> callImageGenerate(AgentTaskRequest request) {
        ...
        // 发布作品模式驱动图片数量：text-cover 只出封面，image-text 出封面+正文配图，full-image 出全图卡片
        String publishMode = "text-cover";
        if (request.getInputs() != null && request.getInputs().get("publishMode") != null) {
            publishMode = String.valueOf(request.getInputs().get("publishMode"));
        }
        ...
        if (!"text-cover".equalsIgnoreCase(publishMode)) {
            images.add(Map.of(
                    "id", "img-002",
        ...
        if ("full-image".equalsIgnoreCase(publishMode)) {
            images.add(Map.of(
                    "id", "img-card-1",
                    "url", "https://picsum.photos/seed/card1/1080/1440",
                    "purpose", "全图卡片-开篇",
                    "style", "极简插画风"
            ));
            images.add(Map.of(
                    "id", "img-card-2",
                    "url", "https://picsum.photos/seed/card2/1080/1440",
                    "purpose", "全图卡片-结尾",
                    "style", "极简插画风"
            ));
        }
        data.put("images", images);
        data.put("totalCount", images.size());
    ...
    public AgentResponse<Map<String, Object>> callPublish(AgentTaskRequest request) {
        ...
        String publishMode = request.getInputs() != null
                && request.getInputs().get("publishMode") != null
                ? String.valueOf(request.getInputs().get("publishMode"))
                : "text-cover";
        data.put("publishMode", publishMode);
