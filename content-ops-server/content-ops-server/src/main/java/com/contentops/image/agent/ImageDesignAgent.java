...
public interface ImageDesignAgent {
     ...
     */
    @UserMessage("""
            输出 JSON 结构必须严格为：{"visualKeywords":[],"directions":[{"name":"","description":"","colorPalette":"","promptPrefix":"","suggestedPositions":"","recommendationScore":3}],"toneAnalysis":""}
    ...
    StyleDirectionResult generateStyleDirections(@MemoryId String memoryId,
                                                 @V("articleTitle") String articleTitle,
                                                 @V("articleContent") String articleContent,
                                                 @V("articleTone") String articleTone,
                                                 @V("targetPlatforms") List<String> targetPlatforms);
     ...
     */
    @UserMessage("""
            禁止复述、解释或讨论工具返回内容；图片生成不可用时直接使用占位 URL，立即返回结构化 JSON。
            输出 JSON 结构必须严格为：{"images":[{"prompt":"","imageUrl":"","style":"","colorTone":"","position":""}],"covers":[{"platform":"","imageUrl":"","width":0,"height":0,"format":""}]}
            重要输出约束：你必须只返回一个合法 JSON 对象，禁止任何前言、解释或对话，禁止使用 markdown 代码块（不要 ```json 包裹）；字符串值内部严禁出现未转义的英文双引号，需要引用时一律使用中文引号「」或“”。
            【阶段二：批量生图】
    ...
    ImageDesignResult generateImages(@MemoryId String memoryId,
                                     @V("confirmedStyle") String confirmedStyle,
                                     @V("articleTitle") String articleTitle,
                                     @V("articleContent") String articleContent,
                                     @V("articleTone") String articleTone,
                                     @V("targetPlatforms") List<String> targetPlatforms);
