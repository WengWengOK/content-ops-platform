...
class ActuatorSecurityFilterTest {
    ...
    @Test
    @DisplayName("Authorization Bearer 携带正确 Key 应放行（Prometheus 抓取兼容）")
    void bearerAuthWithCorrectKey_shouldPassThrough() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/actuator/prometheus");
        request.addHeader("Authorization", "Bearer secret-key-12345");
        MockHttpServletResponse response = new MockHttpServletResponse();
    ...
    @Test
    @DisplayName("Authorization Bearer 携带错误 Key 应返回 401")
    void bearerAuthWithWrongKey_shouldReturn401() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/actuator/prometheus");
